...
$current_page = isset($_GET["list"]) ? intval($_GET["list"]) : redirect("/admin/?page=phone&list=1");

if (($_SERVER['REQUEST_METHOD'] == 'POST') && ($_POST["name"] != "")) {
    $name = $_POST["name"];
    $sql = "select count(id) as total from phone WHERE `name` like '%$name%'";
} else {
...
if (($_SERVER['REQUEST_METHOD'] == 'POST') && ($_POST["name"] != "")) {
    ...
    if (($current_page < 1) || ($current_page > $total_page)) {
        redirect("/admin/?page=phone&list=1");
    }
}
...
?>
<head>
<link rel="stylesheet" href="/admin/pages/phone/styles.css">
</head>
<body>
    <?php
        if (!is_guest()) {
            ...
            <form action="/admin/pages/phone/add.php" method="post" enctype="multipart/form-data">
                <div>
        ...
        <section class="phone-list">
                    ...
                    if ($result_table->num_rows > 0) {
                        while($row = $result_table->fetch_assoc()) {
                            echo "<tr>
                                ...
                                <td><img src=\"/admin/upload/img/phone/" . $row["img"] . "\" alt=\"phone\" width=\"60\" height=\"60\" onclick=\"window.open(this.src)\"></td>";
                                if (!is_guest()) {
                                    echo "<td class='actions'>
                                    <a href='/admin/?page=editPhone&id=" . $row["id"] . "' class='edit-btn'>Sửa</a>
            ...
            <div class="text-center">
                ...
                <ul class="pagination " style="box-shadow: 0px 0px 10px aqua; border-radius: 0px;">
                    ...
                    <li class="page-item"><a class="page-link" href="/admin/?page=phone&list=1">Trang đầu</a></li>
                    <?php endif; ?>
                    ...
                    <?php else: ?>
                        <li class="page-item">
                        <a class="page-link" href="/admin/?page=phone&list=<?php echo $i; ?>"><?php echo $i; ?></a>
                        </li>
                    <?php endif; ?>
                    <?php endfor; ?>

                    <?php if ($current_page < $total_page && $total_page > 1): ?>
                    <li class="page-item"><a class="page-link" href="/admin/?page=phone&list=<?php echo $total_page; ?>">Trang cuối</a></li>
                    <?php endif; ?>
                </ul>
                </nav>
            </div>
    ...
    echo "<script>
        function deleteSubmit(id) {
            if (confirm('Bạn có muốn xóa id [' + id + '] không?')) {
                window.location.href = '/admin/pages/phone/delete.php?id=' + id;
            }
        }
    </script>";
    }
